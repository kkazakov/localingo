...
def load_whisper_model():
    ...
    print("Loading Whisper model: large-v3", flush=True)
...
def build_hf_messages(messages: list[Message]) -> list[dict]:
    ...
    for msg in messages:
        ...
        if item.type == "text":
            ...
            if not item.url:
                raise HTTPException(
                    status_code=400, detail="Image content requires a 'url' field"
                )
            ...
            raise HTTPException(
                status_code=400, detail=f"Unknown content type: {item.type!r}"
            )
...
async def chat_completions(request: Request, body: ChatRequest):
    ...
    if model_type == "gguf":
        if any(
            ...
            raise HTTPException(
                status_code=400, detail="GGUF models do not support image input yet"
            )
